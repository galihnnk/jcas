...
base_script = 'JCAS_OFDM_Multistatic_Core_TB.m';

% Output directories — use absolute paths anchored to script location
script_dir  = fileparts(mfilename('fullpath'));
if isempty(script_dir); script_dir = pwd; end
out_base    = fullfile(script_dir, 'JCAS_Results');
out_comp    = fullfile(script_dir, 'JCAS_Results_Comparison');
out_tables  = fullfile(script_dir, 'JCAS_Results_Comparison', 'comparison_tables');
for d = {out_base, out_comp, out_tables}
    if ~exist(d{1},'dir')
        [ok,msg] = mkdir(d{1});
        if ~ok; error('[SETUP] Cannot create folder: %s\n  Reason: %s', d{1}, msg); end
    end
end
fprintf('[SETUP] Output dirs ready:\n  %s\n  %s\n  %s\n', out_base, out_comp, out_tables);
...
for ei = 1:n_exp
    ...
    sv_ei        = ei;
    sv_exp_label = exp_label;
    sv_bw_hz     = bw_hz;
    sv_n_nodes   = n_nodes;
    sv_topo_mode = topo_mode;
    sv_snr_op    = snr_op;
    sv_R         = R;
    sv_n_exp     = n_exp;
    sv_exp_plan  = exp_plan;
    sv_base      = base_script;
    sv_out_base  = out_base;
    sv_out_comp  = out_comp;
    sv_out_tables= out_tables;
    sv_script_dir= script_dir;
    sv_primary   = primary_ei;

    % Run the base script — clear is skipped because loop_override=true
    run(base_script);

    % Restore runner variables
    ei          = sv_ei;
    exp_label   = sv_exp_label;
    bw_hz       = sv_bw_hz;
    n_nodes     = sv_n_nodes;
    topo_mode   = sv_topo_mode;
    snr_op      = sv_snr_op;
    R           = sv_R;
    n_exp       = sv_n_exp;
    exp_plan    = sv_exp_plan;
    base_script = sv_base;
    out_base    = sv_out_base;
    out_comp    = sv_out_comp;
    out_tables  = sv_out_tables;
    script_dir  = sv_script_dir;
    primary_ei  = sv_primary;
...
if fid==-1; error('[TABLE] Cannot create CompTable_Overall_KPI.csv in: %s', out_tables); end
