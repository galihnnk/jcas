%% =========================================================================
%  JCAS OFDM FULL SIMULATION - FASE 1 s/d FASE 4
%  =========================================================================
%  Peneliti : Galih Nugraha Nurkahfi
%  Institusi: PR Telekomunikasi, BRIN
%  Versi    : 6.0 (Multistatic + EKF + RL Enhanced)
%
%  PERUBAHAN DARI v5.0:
%  ┌---------------------------------------------------------------------┐
...
fprintf('=========================================================\n');
fprintf('  JCAS OFDM Full Simulation | Fase 1-4 | Versi 6.0\n');
...
cfg.plot.fig5 = true;   % Trajectory Animation
...
for k = 1:Nt
    tau_k = 2*target.R_true(k)/cfg.c;  % Range dari sensor 1
    d_k   = round(tau_k*cfg.fs);
    fd_k  = 2*target.v_true(k)*cosd(0)/cfg.lambda;  % Doppler (radial component)
    % [NEW] Hitung angle dari sensor ke target untuk komponen radial
    dx = target.x(k) - sensor(1).x;
    dy = target.y(k) - sensor(1).y;
    angle_k = atan2(dy, dx);  % sudut dari sensor ke target
    % Komponen radial kecepatan (positif = mendekat)
    v_radial_k = target.v_true(k) * cos(angle_k);
    fd_k = 2*v_radial_k/cfg.lambda;
    A_k  = sqrt(target.RCS(k)/max(target.RCS));
    if d_k < N_tx
...
rx_sensing_grid = demod_ofdm(rx_final_sensing, Nsc, Nsym, cfg.Ncp, Lsym);
rx_comm_grid    = demod_ofdm(rx_comm_base,     Nsc, Nsym, cfg.Ncp, Lsym);
...
if n_sensors > 1
    for si = 2:n_sensors
        rx_si_grid = demod_ofdm(rx_final_extra{si}, Nsc, Nsym, cfg.Ncp, Lsym);
        ...
        for lane_l=0:cfg.rl.n_lane_levels-1
            ...
            for a_idx=1:cfg.rl.n_actions
                ...
                if Ns_q>0
                    ...
                    % [NEW] Pd estimasi dari RMSE (normcdf approximation)
                    precomp_pd(s_idx,a_idx) = 2*normcdf(delta_R/2, 0, max(rmse_q,0.01)) - 1;
                else
...
[~,BER] = biterr(tx_bits, rx_bits_trim);
...
for si = 1:N_snr
    ...
    for tr = 1:cfg.Ntrials
        ...
        for k=1:Nt
            ...
            Ak=sqrt(target.RCS(k)/max(target.RCS));
            if dk<N_tx
        ...
        % MC hanya menggunakan sensor 1 (RSU) untuk konsistensi dengan standar
        % Pd yang ditampilkan adalah lower bound - multistatic Pd lebih tinggi
        Hg_s = demod_ofdm(rx_r+nz_s,Nsc,Nsym,cfg.Ncp,Lsym)./(tx_grid+eps_div);
        RD_mc = fftshift(fft(ifft(Hg_s,Nsc,1),Nsym,2),2);
        [cfar_mc, ~] = cfar_2d(abs(RD_mc), cfg.cfar.guard_cells, cfg.cfar.train_cells, cfg.cfar.Pfa);

        for k=1:Nt
            ir_k = min(round(target.R_true(k)/delta_R)+1, Nsc);
            iv_k = round(Nsym/2) + round(target.v_true(k)/delta_v);
            iv_k = max(1, min(Nsym, iv_k));
            r1=max(1,ir_k-cfg.cfar.guard_cells); r2=min(Nsc,ir_k+cfg.cfar.guard_cells);
            c1=max(1,iv_k-cfg.cfar.guard_cells); c2=min(Nsym,iv_k+cfg.cfar.guard_cells);
            if any(any(cfar_mc(r1:r2,c1:c2))); n_detect=n_detect+1; end
            n_target_cells=n_target_cells+1;
            rp_t=mean(abs(ifft(Hg_s,Nsc,1)),2);
            ...
            [~,ir_l]=max(rp_t(wlo:whi)); ir_l=ir_l+wlo-1;
            r_errs(k,tr)=(ir_l-1)*delta_R-target.R_true(k);
        end
        n_fa=n_fa+max(0,sum(cfar_mc(:))-n_detect/cfg.Ntrials);

        Hg_c=demod_ofdm(rx_c+nz_c,Nsc,Nsym,cfg.Ncp,Lsym);
...
if cfg.enable.fase4
    ...
    for bi2 = 1:n_boot
        ...
        Hg_boot = demod_ofdm(rx_boot,Nsc,Nsym,cfg.Ncp,Lsym)./(tx_grid+eps_div);
...
if cfg.plot.fig3
    ...
    for k=1:Nt
        ...
        node_pd(k) = max(0,min(1, 2*normcdf(delta_R/2,0,max(rmse_k,0.01))-1));
    end
    yyaxis left;
...
if cfg.export.csv
    ...
    % ---- CSV 1: Paper Summary ----
    fid=fopen(fullfile(export_dir,'CSV01_Paper_Summary.csv'),'w');
    if fid~=-1
    ...
    fid=fopen(fullfile(export_dir,'CSV02_Pareto_Sweep.csv'),'w');
    if fid~=-1
    ...
    % ---- CSV 3: Pd vs SNR ----
    fid=fopen(fullfile(export_dir,'CSV03_Pd_vs_SNR.csv'),'w');
    if fid~=-1
    ...
    % ---- CSV 4: Per-Node RMSE ----
    fid=fopen(fullfile(export_dir,'CSV04_PerNode_RMSE.csv'),'w');
    if fid~=-1
    ...
    % ---- CSV 5: BW Comparison ----
    if cfg.BW_compare
        fid=fopen(fullfile(export_dir,'CSV05_BW_Comparison.csv'),'w');
        if fid~=-1
    ...
    if cfg.enable.fase4
        fid=fopen(fullfile(export_dir,'CSV06_RL_Training.csv'),'w');
        if fid~=-1
            hdr_str='Episode';
    ...
    if cfg.enable.fase4 && isfield(deep,'ci_mean')
        fid=fopen(fullfile(export_dir,'CSV07_Statistical_Significance.csv'),'w');
        if fid~=-1
    ...
    if cfg.enable.fase4 && isfield(deep,'sens_wc')
        fid=fopen(fullfile(export_dir,'CSV08_Sensitivity_Analysis.csv'),'w');
        if fid~=-1
    ...
    if cfg.enable.fase4 && isfield(deep,'conv_mavg')
        fid=fopen(fullfile(export_dir,'CSV09_Convergence.csv'),'w');
        if fid~=-1
            hdr_str='Episode';
    ...
    if cfg.enable.fase4 && isfield(deep,'abl_names')
        fid=fopen(fullfile(export_dir,'CSV10_Ablation.csv'),'w');
        if fid~=-1
    ...
    if cfg.enable.fase4 && isfield(deep,'gen_names')
        fid=fopen(fullfile(export_dir,'CSV11_Generalization.csv'),'w');
        if fid~=-1
    ...
    fid=fopen(fullfile(export_dir,'CSV12_System_Config.csv'),'w');
    if fid~=-1
...
fprintf('\n=========================================================\n');
fprintf('          FINAL SUMMARY - JCAS v6.0\n');
fprintf('=========================================================\n');
fprintf(' Scenario : %s | %d nodes | fc=%.1f GHz | BW=%.0f MHz\n', ...
    cfg.v2x.mobility_model, Nt, cfg.fc/1e9, cfg.BW/1e6);
fprintf(' Mode     : Multistatic-%s | sensors=%d\n', cfg.multi.mode, n_sensors);
fprintf(' Nodes    : %d resolvable | %d overlap (delta_R=%.2f m)\n', ...
    Nt-n_overlap, n_overlap, delta_R);

fprintf('\n--- SENSING (EKF 2D) --------------------------------\n');
fprintf('  CRB Range  AWGN / Multipath : %.4f m / %.4f m (+%.1f dB)\n', ...
    CRB_R_awgn, CRB_R_mp, degrad_R_dB);
fprintf('  EKF RMSE   raw -> filtered  : %.3f m -> %.3f m (%+.1f%%)\n', ...
    mean(rmse_raw_all), mean(rmse_kalm_all), ...
    mean((1-rmse_kalm_all./max(rmse_raw_all,1e-6))*100));
if n_sensors > 1
    fprintf('  Triangulation avg pos error : %.2f m\n', nanmean(tri_pos_err));
end
snr_op_idx = find(cfg.SNR_sweep == cfg.SNR_dB, 1);
pd_op = 0;
if ~isempty(snr_op_idx)
    pd_op = Pd_arr(snr_op_idx)*100;
    fprintf('  Pd @ SNR_op=%.0f dB         : %.2f%%  [5GAA>=%.0f%%: %s]\n', ...
        cfg.SNR_dB, pd_op, saa_pd, deal_ternary(pd_op>=saa_pd,'PASS','FAIL'));
end
fprintf('  Pfa                         : %.3f%%  [5GAA<=%.0f%%: %s]\n', ...
    cfg.cfar.Pfa*100, saa_pfa, deal_ternary(cfg.cfar.Pfa*100<=saa_pfa,'PASS','FAIL'));
fprintf('  Range Resolution            : %.2f m  [5GAA<=%.1f m: %s]\n', ...
    delta_R, saa_range_res, deal_ternary(delta_R<=saa_range_res,'PASS','FAIL'));
fprintf('  SI after mitigation         : %.1f dBc  [<=-20 dBc: %s]\n', ...
    SI_dBc_after, deal_ternary(SI_dBc_after<=-20,'PASS','FAIL'));

if cfg.BW_compare
    fprintf('\n--- BANDWIDTH COMPARISON ----------------------------\n');
    for bi = 1:length(cfg.BW_list)
        fprintf('  BW=%.0f MHz : CRB=%.4f m | RMSE=%.2f m\n', ...
            cfg.BW_list(bi)/1e6, BW_crb(bi), BW_rmse_avg(bi));
    end
end
...
if cfg.enable.fase4
    fprintf('\n--- RESOURCE ALLOCATION -----------------------------\n');
    fprintf('  SNR_ra=%.1f dB | State=%d | %s | Channel=%s\n', SNR_ra_dB, state_current, ...
        deal_ternary(is_LoS,'LoS','NLoS'), deal_ternary(ch_is_busy,'BUSY','IDLE'));
    fprintf('  %-14s %8s %12s %12s %8s\n', ...
        'Strategy','Nc/Nsc','Cap [Mbps]','vs Static','5GAA');
    all_n = [{'Static','WaterFill','Adaptive'}, run_algos];
    all_c = [best_cap_s, best_cap_w, best_cap_a, best_cap_rl];
    all_r = [best_ratio_s, best_ratio_w, best_ratio_a, best_ratio_rl];
    for mi = 1:length(all_n)
        g  = (all_c(mi)-best_cap_s)/max(best_cap_s,eps)*100;
        pf = deal_ternary(all_c(mi)>=saa_tput,'PASS','FAIL');
        fprintf('  %-14s %8.2f %12.2f %+11.1f%% %8s\n', ...
            all_n{mi}, all_r(mi), all_c(mi), g, pf);
    end
end

fprintf('\n--- COMMUNICATION -----------------------------------\n');
fprintf('  Throughput : %.2f Mbps  [5GAA>=%.1f Mbps: %s]\n', ...
    capacity_bps/1e6, saa_tput, deal_ternary(capacity_bps/1e6>=saa_tput,'PASS','FAIL'));
fprintf('  BER        : %.5f  [<1e-3: %s]\n', ...
    BER, deal_ternary(BER<1e-3,'PASS','FAIL'));
fprintf('  PAPR       : %.2f dB\n', PAPR_dB);
fprintf('\n=========================================================\n');
...
function reward = compute_rl_reward_v2(cap_val,cap_max,rmse_val,rmse_worst,...
    ...
    % Penalty bertahap (lebih smooth dari v5)
    if ~isfinite(rmse_val)||rmse_val>0.5; penalty=4;
    elseif rmse_val>0.3; penalty=1.5;
    elseif rmse_val>0.1; penalty=0.5;
    else; penalty=0; end
